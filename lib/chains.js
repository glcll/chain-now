/**
 * Chain registry for all CRE-supported networks.
 * Maps human-readable chain names to CRE selector names, chain IDs,
 * explorer URLs, and DataRegistry contract addresses.
 */
...
function getChainBySelector(selectorName) {
  return Object.values(CHAINS).find(c => c.selectorName === selectorName) || null;
}

function listChains({ testnets = true } = {}) {
...
function explorerTxUrl(chainSlug, txHash) {
  const chain = CHAINS[chainSlug];
  if (!chain) return null;
  return `${chain.explorer}/tx/${txHash}`;
}

module.exports = { CHAINS, getChain, getChainBySelector, listChains, explorerTxUrl };
