...
function decodeJson2(input) {
  const text = new TextDecoder().decode(input);
  return JSON.parse(text);
}
var onHttpTrigger = (runtime2, payload) => {
  const request = decodeJson2(payload.input);
  runtime2.log(`Write request: key=${request.key}`);
  ...
  const reportData = encodeAbiParameters(parseAbiParameters("bytes32 key, bytes value"), [keyBytes32, valueBytes]);
  runtime2.log("Generating signed report...");
  ...
  runtime2.log("Submitting to blockchain...");
